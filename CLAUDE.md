...
- `frontend/app/tutorial/`: `page.tsx` (server) + `TutorialClient.tsx` (builds all cards from `data.json` on mount, no step gating) + `TutorialWelcomeModal.tsx` + `explain-content.ts` (technique copy to `InfoSection[]`, via `explainSectionsFor`/`explainSectionsByCardType`) + `steps.ts` (types) + `load-steps.ts` (parses `content/step-*.md`) + `data.json` (pre-computed results)
