...
Every run on the logit lens, DLA, patching, and attention head analysis is cached. This means that rerunning the same configuration allows one to get the cached result instantly at no cost. Caches are scoped to your account; another user running the same prompt does not see or share your cache entries. (uh... check this?)
