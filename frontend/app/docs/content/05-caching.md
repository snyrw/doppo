...
Every run on the logit lens, DLA, patching, and attention head analysis is cached. This means that rerunning the same configuration allows one to get the cached result instantly at no cost. Caches are global, which means that users will have a quick time running popular prompts.
