...
export async function POST(request: NextRequest) {
  ...
  const cacheKey = createHash("sha256")
    .update(`${userId}:${modelName}:${cleanPrompt}:${corruptedPrompt}:${resolvedPosition}:${resolvedToken}:${resolvedContrastive}`)
    .digest("hex");

  const cached = await db.select({ r2Key: attributionCache.r2Key }).from(attributionCache).where(eq(attributionCache.id, cacheKey)).limit(1);
  if (cached.length > 0 && cached[0].r2Key) {
    const data = await getHeatmap(cached[0].r2Key);
    db.update(attributionCache).set({ lastAccessedAt: new Date() }).where(eq(attributionCache.id, cacheKey)).catch(console.error);
  ...
  const spawnRes = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/job/spawn-attribution`, {
    method: "POST",
    headers: backendHeaders({ "Content-Type": "application/json" }),
    body: JSON.stringify({ prompt: cleanPrompt, corrupted_prompt: corruptedPrompt, model_name: modelName, target_position: targetPosition, target_token: targetToken, contrastive_token: contrastiveToken }),
  });
  if (!spawnRes.ok) {
  ...
  await db.insert(activeJobs).values({
    id: job_id, userId, gpuTier: resolvedTier, jobType: "attribution", modelName,
    cacheKey,
    cachePayload: JSON.stringify({ modelName, prompt: cleanPrompt, corruptedPrompt, targetPosition: resolvedPosition, targetToken: resolvedToken }),
    startedAt: new Date(),
  });
