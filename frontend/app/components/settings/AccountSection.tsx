...
const btnCls = "btn-accent cursor-pointer px-3 py-1.5 text-[13px] font-semibold disabled:opacity-50";
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="flex flex-col gap-2">
        <label className="text-[10px] font-semibold text-muted">Display name</label>
        <input className={inputCls} value={name} onChange={(e) => setName(e.target.value)} />
        <button className={btnCls + " self-start"} onClick={saveName}>Save name</button>
      </div>
      <div className="flex flex-col gap-2">
        <label className="text-[10px] font-semibold text-muted">Email — current: {session?.user.email}</label>
      ...
      {showPw && (
        <div className="flex flex-col gap-2">
          <label className="text-[10px] font-semibold text-muted">Change password</label>
