      ...
      >
        ...
        {data && (
          ...
          >
            ...
            >←</button>
            <span style={{ fontSize: 10, fontFamily: "var(--font-syne-mono), monospace", color: "var(--color-text)", minWidth: 28, textAlign: "center", fontVariantNumeric: "tabular-nums" }}>
              L{currentLayer}
            </span>
            <button
              onClick={() => handleLayerChange(1)}
        ...
        <>
          ...
          >
            {infoContent.type === "confirm" && (
              <span style={{ fontSize: 9, fontFamily: "var(--font-syne-mono), monospace", color: "var(--color-accent)", fontWeight: 600, whiteSpace: "nowrap" }}>
                {infoContent.text}
              </span>
            )}
            {infoContent.type === "focused" && (
              <span style={{ fontSize: 9, fontFamily: "var(--font-syne-mono), monospace", color: "var(--color-text-muted)", whiteSpace: "nowrap" }}>
                {infoContent.text}
              </span>
            )}
            ...
            {infoContent.type === "idle" && (
              <span style={{ fontSize: 9, color: "var(--color-text-muted)", fontFamily: "var(--font-syne-mono), monospace" }}>
          ...
          {pinnedHeads.length > 0 && (
            <div style={{
              ...
              >
                <span style={{ fontSize: 9, fontWeight: 700, color: "var(--color-text-muted)", fontFamily: "var(--font-syne-mono), monospace", letterSpacing: "0.08em" }}>
                  PINNED
                </span>
                <span style={{ fontSize: 9, color: "var(--color-text-muted)", fontFamily: "var(--font-syne-mono), monospace" }}>
                ...
                <button
                  ...
                  style={{ fontSize: 9, color: "var(--color-text-muted)", background: "none", border: "none", cursor: "pointer", padding: 0, fontFamily: "var(--font-syne-mono), monospace" }}
                >
                  clear all
                </button>
              </div>
                  ...
                  {pinnedHeads.map(({ layer, head }) => (
                    <div key={`${layer}-${head}`} style={{ flexShrink: 0, display: "flex", flexDirection: "column" }}>
                      <div style={{
                        ...
                        <span style={{ fontSize: 8, fontFamily: "var(--font-syne-mono), monospace", color: "var(--color-accent)", fontWeight: 700 }}>
                          L{layer}·H{head}
                        </span>
                        <button
                          onPointerDown={e => e.stopPropagation()}
